...
from DataLoaders import load_FrozenLakeSlippery
...
DEBUG= False
opt_alg_name= 'ES'
problem_name= 'FrozenLakeSlippery'
model_type= 'QML'
...
def ActionSelector(logits):
    ...
    sol= np.empty(len(logits), dtype=int)
    for i, l in enumerate(logits):
        if np.abs(l[0]) >= np.abs(l[1]):
            sol[i]= 0 if l[0]>= 0 else 2
        else:
            sol[i]= 3 if l[1]>= 0 else 1
    return sol
...
env_data= load_FrozenLakeSlippery()
...
model_param= {
    ...
    'observables': 'XZ', # String containing the observables to be used
